...
class InternalAuthService(internal_auth_pb2_grpc.InternalAuthServiceServicer):
        ...
        try:
            ...
            if company_id:
                ...
                return internal_auth_pb2.VerifyTokenResponse(
                    valid=True, tenant_id=str(company_id)
                )
            else:
            ...
            context.set_details("Internal server error during token verification")
            return internal_auth_pb2.VerifyTokenResponse(valid=False)
    ...
    def _verify_token_db_sync(self, token: str):
        ...
        with self.session_factory() as db:
            ...
            company = (
                db.query(Company)
                .filter(Company.deployment_token == token, Company.is_active)
                .first()
            )
            if company:
                return company.id
