...
class ScanService(scan_pb2_grpc.ScanServiceServicer):
    ...
    async def UpdateScanStatus(self, request, context):
        ...
        if not token or not token.startswith("ag_"):
            ...
            context.set_details("Agent deployment token required")
            return scan_pb2.UpdateScanStatusResponse()
        ...
        if not company_id:
            context.set_code(grpc.StatusCode.UNAUTHENTICATED)
            context.set_details("Invalid or inactive deployment token")
            return scan_pb2.UpdateScanStatusResponse()
        ...
        if not row:
            ...
            context.set_details("Scan not found or access denied")
            return scan_pb2.UpdateScanStatusResponse()
...
def _execute_status_update(scan_id, status):
    conn = get_db_connection()
    try:
        cur = conn.cursor()
        cur.execute(
        ...
        broadcaster.broadcast("scan", (scan_id, status))
    finally:
        conn.close()
