# Graph Report - Aegis-AI-Brain  (2026-09-23)

## Corpus Check
- 108 files · ~41,776 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1315 nodes · 2105 edges · 117 communities (100 shown, 17 thin omitted)
- Extraction: 82% EXTRACTED · 18% INFERRED · 0% AMBIGUOUS · INFERRED: 383 edges (avg confidence: 0.7)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `5a66f61a`
...
- [[_COMMUNITY_20260525_0001_schema_v2_2.py|20260525_0001_schema_v2_2.py]]
...
1. `User` - 52 edges
2. `GraphDrivenPentestWorkflow` - 44 edges
3. `CompanyService` - 33 edges
4. `AuthService` - 29 edges
5. `Company` - 27 edges
6. `OnboardingInvitation` - 22 edges
7. `Neo4jSandboxTopologyService` - 21 edges
8. `get_db_connection()` - 19 edges
9. `PentestWorkflow` - 19 edges
...
Cohesion: 0.08
Nodes (39): GraphDrivenPentestWorkflow, Exception, failing_update_scan_status(), mock_build_sandbox_topology(), mock_capture_run_targeted_pentest(), mock_check_task_queue_pollers(), mock_check_task_queue_pollers_unavailable(), mock_create_sandbox() (+31 more)
...
Cohesion: 0.09
Nodes (18): Company, Company model mapped to the 'companies' table., User model mapped to the 'users' table., User, test_company_summary_includes_current_company_metadata(), test_create_company_success(), test_create_user_success(), test_list_companies_success() (+10 more)

### Community 4 - "User"
Cohesion: 0.08
Nodes (25): License, License model mapped to the 'licenses' table., Evidence, Scan model mapped to the 'scans' table., Vulnerability model mapped to the 'vulnerabilities' table., Evidence model mapped to the 'evidences' table., Scan, Vulnerability (+17 more)
...
### Community 7 - "$ref"
Cohesion: 0.08
Nodes (25): $ref, type, type, type, type, type, env_vars, id (+17 more)

### Community 8 - "CompanyService"
Cohesion: 0.11
Nodes (16): CreateCompanyRequest, CreateCompanyResponse, DeclarativeBase, OnboardCompanyRequest, OnboardCompanyResponse, CompanyCreateError, CompanyService, CompanyService handles company creation and administrative listing. (+8 more)
...
Cohesion: 0.21
Nodes (25): FPDF, test_ensure_space(), test_format_loot_data(), test_safe_text_none(), test_truncate_text_long(), build_report(), _count_by_severity(), _ensure_space() (+17 more)

### Community 11 - "BillingService"
Cohesion: 0.11
Nodes (18): OnboardingInvitation, One-time token used to activate an onboarded owner account., hash_password(), Verifies a plain-text password against a stored bcrypt hash.      Args:, Hashes a plain-text password using the bcrypt algorithm.      Args:         pass, verify_password(), test_login_db_error(), test_login_inactive_user() (+10 more)
...
Cohesion: 0.13
Nodes (8): Connection, get_db_connection(), Establishes a raw connection to the PostgreSQL database (legacy)., normalize_persisted_target_image(), ScanService, to_pb_timestamp(), _extract_loot_fields(), VulnerabilityService
...
Cohesion: 0.19
Nodes (6): build_sandbox_topology(), Neo4jSandboxTopologyService, test_build_sandbox_topology_includes_known_workload_routes(), test_build_sandbox_topology_loads_database_schemas_for_route_selected_containers(), test_build_sandbox_topology_maps_containers_to_deployer_payload(), test_build_sandbox_topology_uses_default_http_port_when_missing()
...
Cohesion: 0.22
Nodes (16): Path, _extract_topology(), _format_path(), Any, SandboxTopologyValidationError, _schema(), _schema_path(), validate_sandbox_topology() (+8 more)
...
### Community 21 - "object"
Cohesion: 0.12
Nodes (12): object, BillingServiceStub, Constructor.          Args:             channel: A grpc.Channel., BillingService handles token management and consumption history., InternalAuthService, InternalAuthServiceServicer, InternalAuthServiceStub, Constructor.          Args:             channel: A grpc.Channel. (+4 more)
...
Cohesion: 0.15
Nodes (13): type, $defs, initContainer, stringMap, workload, additionalProperties, required, type (+5 more)
...
Cohesion: 0.18
Nodes (11): _execute_generate_and_store_pdf_report(), generate_and_store_pdf_report(), Generates PDF bytes in memory and stores them in scans.report_pdf., Generates a structured PDF report in memory and stores it in scans.report_pdf., Test generating and storing a PDF report successfully., Test exception when storing PDF for a missing scan., Test PDF report activity when DB connection cannot be established., test_generate_and_store_pdf_report_db_fail() (+3 more)

### Community 33 - "properties"
Cohesion: 0.29
Nodes (4): get_identity(), Securely extracts identity from verified context or gRPC metadata fallback., Decorator to inject identity into the handler.     Supports both async functions, with_identity()
...
Cohesion: 0.17
Nodes (12): items, type, additionalProperties, properties, type, databaseForeignKey, columns, referenced_columns (+4 more)
...
### Community 39 - "type"
Cohesion: 0.18
Nodes (11): _execute_update_scan_crew_report(), Internal helper to persist the latest CrewAI JSON and markdown output., Stores the latest CrewAI report artifacts for a scan., update_scan_crew_report(), Test saving vulnerabilities and their evidences., Test saving empty vulnerabilities list., test_build_sandbox_topology_includes_database_schemas(), test_save_vulnerabilities_empty() (+3 more)

### Community 40 - "config.py"
Cohesion: 0.06
Nodes (32): Client, _artifact_payload(), download_minio_artifact(), parse_minio_reference(), check_task_queue_pollers(), serve(), AuthInterceptor, gRPC interceptor for JWT validation and verified identity injection. (+24 more)

### Community 41 - "update_scan_status"
Cohesion: 0.17
Nodes (12): _execute_status_update(), Internal helper to execute the SQL update., Updates the status of a specific scan in the PostgreSQL database., update_scan_status(), Test intermediate status updates do not set completed_at., Test activity throwing exception when rowcount is 0., Test activity throwing exception when DB is down., Test updating the status of a scan successfully. (+4 more)

### Community 42 - "ping_pb2_grpc.py"
Cohesion: 0.17
Nodes (7): BillingServiceServicer, BillingService handles token management and consumption history., GetBalance retrieves the current token balance for a company., GetLedger retrieves the history of token transactions for a company., AdjustTokens adds or subtracts tokens from a company's balance., PreFlightCheck estimates the cost of a scan and checks if the balance is suffici, GetUsageStats retrieves aggregated consumption data for graphs.

### Community 44 - "🏗️ Core Models"
Cohesion: 0.18
Nodes (11): items, type, $ref, minLength, type, properties, command, env (+3 more)

### Community 45 - "🏗️ Modèles Principaux"
Cohesion: 0.20
Nodes (10): items, type, items, type, $ref, items, type, config_files (+2 more)
...
### Community 48 - "name"
Cohesion: 0.22
Nodes (9): additionalProperties, properties, type, databaseIndex, minLength, type, name, unique (+1 more)

### Community 49 - "db_activities.py"
Cohesion: 0.29
Nodes (9): _dedupe_evidences(), _dedupe_vulnerabilities(), _execute_save_vulnerabilities(), Internal helper to insert vulnerabilities and their evidences., Saves a list of vulnerabilities and their evidences to the PostgreSQL database., save_vulnerabilities(), _stable_json(), Duplicate findings/evidences in a single run are stored once. (+1 more)
...
### Community 64 - "test_rbac_visibility.py"
Cohesion: 0.29
...
Cohesion: 0.29
Nodes (7): items, type, properties, connections, routes, items, type

### Community 67 - "sandbox-topology.schema.json"
Cohesion: 0.33
Nodes (5): Intended use, 🧠 LLM Payload Assistance, Prompt data, Safety boundaries, Worker interaction
...
Cohesion: 0.33
Nodes (5): 🕸️ Neo4j Topology Graph, Operational rule, Purpose, Typical nodes, Typical relationships

### Community 70 - "VulnerabilityServiceServicer"
Cohesion: 0.33
Nodes (5): 🧠 Assistance LLM aux payloads, Données de prompt, Interaction avec le worker, Limites de sûreté, Usage prévu

### Community 71 - "test_auth_init.py"
Cohesion: 0.33
Nodes (5): 🕸️ Graphe de topologie Neo4j, Nœuds typiques, Objectif, Relations typiques, Règle opérationnelle
...
### Community 80 - "20260525_0001_schema_v2_2.py"
Cohesion: 0.50
Nodes (4): description, items, type, databaseSchemas

### Community 81 - ".GetMe"
Cohesion: 0.50
Nodes (4): description, items, type, deployments
...
### Community 85 - ".UpdatePassword"
Cohesion: 0.50
Nodes (4): services, description, items, type

### Community 86 - ".UpdateProfile"
Cohesion: 0.50
...
### Community 96 - "auth_pb2.py"
Cohesion: 0.67
Nodes (3): additionalProperties, type, databaseSchema

### Community 97 - "billing_pb2.py"
Cohesion: 0.67
Nodes (3): items, type, empty_dirs
...
## Knowledge Gaps
- **190 isolated node(s):** `entrypoint.sh script`, `aegis-ai-brain`, `$schema`, `$id`, `title` (+185 more)
...
- **Why does `AuthService` connect `agent_watcher.py` to `properties`, `.UpdatePassword`, `.UpdateProfile`, `properties`, `.Logout`, `.Refresh`, `CompanyService`, `BillingService`, `.SetupPassword`, `.UpdateEmail`, `neo4j-graph.md`, `agent_pb2.py`?**
  _High betweenness centrality (0.033) - this node is a cross-community bridge._
- **Why does `$defs` connect `properties` to `auth_pb2.py`, `ScanServiceServicer`, `OnboardingInvitation`, `AuthService`, `$defs`, `name`, `properties`, `Base`, `test_internal_auth.py`, `The Aegis AI Brain (Orchestrator)`, `Le Brain Aegis AI (Orchestrateur)`, `externalMock`?**
  _High betweenness centrality (0.028) - this node is a cross-community bridge._
- **Why does `User` connect `properties` to `agent_watcher.py`, `User`, `CompanyService`, `container_port`, `BillingService`, `get_session_factory`, `CompanyService`, `service`?**
...
- **Are the 11 inferred relationships involving `GraphDrivenPentestWorkflow` (e.g. with `start_worker()` and `SandboxTopologyValidationError`) actually correct?**
  _`GraphDrivenPentestWorkflow` has 11 INFERRED edges - model-reasoned connections that need verification._
