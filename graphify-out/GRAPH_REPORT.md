# Graph Report - brain-pentest-stabilization  (2026-09-15)

## Corpus Check
- 105 files · ~39,150 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1258 nodes · 1953 edges · 116 communities (95 shown, 21 thin omitted)
- Extraction: 83% EXTRACTED · 17% INFERRED · 0% AMBIGUOUS · INFERRED: 333 edges (avg confidence: 0.71)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `99b06dfe`
...
- [[_COMMUNITY_TestDatabaseConnection|TestDatabaseConnection]]
...
- [[_COMMUNITY_.ListAuditLogs|.ListAuditLogs]]
- [[_COMMUNITY_.ListCompanies|.ListCompanies]]
...
- `test_generate_and_store_pdf_report_passes_crewai_markdown_to_builder()` --indirect_call--> `generate_and_store_pdf_report()`  [INFERRED]
...
Cohesion: 0.09
Nodes (31): GraphDrivenPentestWorkflow, Exception, failing_update_scan_status(), mock_build_sandbox_topology(), mock_create_sandbox(), mock_destroy_sandbox(), mock_download_invalid_topology_artifact(), mock_download_minio_artifact() (+23 more)
...
Cohesion: 0.08
Nodes (19): OnboardingInvitation, One-time token used to activate an onboarded owner account., hash_password(), Verifies a plain-text password against a stored bcrypt hash.      Args:, Hashes a plain-text password using the bcrypt algorithm.      Args:         pass, verify_password(), test_login_db_error(), test_login_inactive_user() (+11 more)

### Community 4 - "User"
Cohesion: 0.08
Nodes (16): User model mapped to the 'users' table., User, test_update_email_conflict(), test_update_password_invalid_old(), test_update_profile_success(), test_company_summary_includes_current_company_metadata(), test_create_company_success(), test_list_companies_success() (+8 more)
...
Cohesion: 0.09
Nodes (20): CreateCompanyRequest, CreateCompanyResponse, ListAuditLogsRequest, ListAuditLogsResponse, OnboardCompanyRequest, OnboardCompanyResponse, str, CompanyCreateError (+12 more)

### Community 9 - "AgentService"
Cohesion: 0.25
Nodes (5): InternalAuthService, gRPC service for verifying agent deployment tokens. Caching is handled by the AP, Synchronously verifies an agent token and returns the company_id., VerifyTokenRequest, VerifyTokenResponse
...
Cohesion: 0.09
Nodes (20): datetime, DeclarativeBase, Builds the SQLAlchemy DB URL securely., get_identity(), Securely extracts identity from verified context or gRPC metadata fallback., Decorator to inject identity into the handler.     Supports both async functions, with_identity(), Agent model mapped to the 'agents' table. (+12 more)

### Community 13 - "get_db_connection"
Cohesion: 0.23
Nodes (4): Connection, get_db_connection(), Establishes a raw connection to the PostgreSQL database (legacy)., ScanService
...
Cohesion: 0.11
Nodes (19): $ref, type, type, type, type, type, env_vars, id (+11 more)
...
Cohesion: 0.20
Nodes (5): build_sandbox_topology(), Neo4jSandboxTopologyService, test_build_sandbox_topology_includes_known_workload_routes(), test_build_sandbox_topology_maps_containers_to_deployer_payload(), test_build_sandbox_topology_uses_default_http_port_when_missing()
...
Cohesion: 0.23
Nodes (13): MockContext, test_ping_service(), test_scan_service_list(), test_scan_service_report(), test_scan_service_start(), test_scan_service_start_failure_compensation(), test_scan_service_start_persists_long_topology_target(), test_scan_service_start_persists_short_topology_target() (+5 more)
...
### Community 26 - "db.py"
Cohesion: 0.13
Nodes (15): items, type, description, items, type, description, items, type (+7 more)
...
Cohesion: 0.27
Nodes (9): AuthInterceptor, gRPC interceptor for JWT validation and verified identity injection., MockHandlerCallDetails, test_auth_interceptor_expired_token(), test_auth_interceptor_invalid_token(), test_auth_interceptor_malformed_header(), test_auth_interceptor_no_auth_header(), test_auth_interceptor_spoofing_prevention() (+1 more)

### Community 29 - "properties"
Cohesion: 0.13
Nodes (14): db_session(), Tests first-login invitation persistence and relationship to a user., Tests the basic License model., In-memory SQLite session for testing models., Tests the Company model creation and its basic fields., Tests the User model, including the role enum and default fields., Tests complex relationships between companies and users (owner/members)., Tests the RefreshToken model and its relationship to a user. (+6 more)
...
Cohesion: 0.09
Nodes (17): URL, _build_db_url(), get_engine(), get_session(), get_session_factory(), Lazily initializes and returns the SQLAlchemy engine., Lazily initializes and returns the session factory., Dependency for providing a SQLAlchemy session. (+9 more)

### Community 36 - "ScanService"
Cohesion: 0.15
Nodes (13): properties, source, source_name, sourceName, target, target_name, targetName, type (+5 more)
...
Cohesion: 0.24
Nodes (9): identify_attack_targets(), _artifact_payload(), download_minio_artifact(), parse_minio_reference(), build_sandbox_topology(), test_artifact_payload_maps_topology_json_to_sandbox_request(), test_download_minio_artifact_reads_object_and_returns_deployable_payload(), test_parse_minio_reference_supports_explicit_and_default_bucket() (+1 more)
...
Cohesion: 0.17
Nodes (12): type, additionalProperties, type, $defs, connection, stringMap, workload, additionalProperties (+4 more)
...
### Community 46 - "$defs"
Cohesion: 0.20
Nodes (4): normalize_persisted_target_image(), to_pb_timestamp(), _extract_loot_fields(), VulnerabilityService
...
### Community 48 - "name"
Cohesion: 0.20
Nodes (9): gRPC handler for token verification., Valid active token returns the associated company_id., Unknown or inactive token returns None., DB error returns None gracefully (does not raise)., Invalid token formats fail before DB lookup., test_verify_token_db_exception(), test_verify_token_invalid_format_skips_database(), test_verify_token_not_found() (+1 more)

### Community 49 - "db_activities.py"
Cohesion: 0.25
Nodes (8): _execute_save_vulnerabilities(), Internal helper to insert vulnerabilities and their evidences., Saves a list of vulnerabilities and their evidences to the PostgreSQL database., save_vulnerabilities(), Test saving vulnerabilities and their evidences., Test saving empty vulnerabilities list., test_save_vulnerabilities_empty(), test_save_vulnerabilities_success()
...
Cohesion: 0.31
Nodes (5): serve(), PingService, test_grpc_server_mtls_fails_when_certificate_missing(), test_grpc_server_mtls_requires_client_certificate(), test_grpc_server_serve_registration()
...
Cohesion: 0.13
Nodes (19): _execute_generate_and_store_pdf_report(), _execute_update_scan_debug_bundle(), generate_and_store_pdf_report(), Generates PDF bytes in memory and stores them in scans.report_pdf., Generates a structured PDF report in memory and stores it in scans.report_pdf., Internal helper to persist the latest sandbox debug bundle reference., Stores the latest Deployer debug bundle reference for a scan., update_scan_debug_bundle() (+11 more)
...
Cohesion: 0.33
Nodes (3): Test returning a database connection on success., Test raising ConnectionError when database connection fails., TestDatabaseConnection
...
Cohesion: 0.18
Nodes (8): LoginRequest, LoginResponse, Authenticates user and returns Access + Refresh tokens., Synchronous part of Login logic., Verify session_factory is only created on first access., Verify AuthService initializes without triggering DB configuration., test_auth_service_init_is_truly_lazy(), test_auth_service_lazy_factory()

### Community 69 - "test_search_coverage.py"
Cohesion: 0.50
Nodes (4): description, items, type, externalMocks

### Community 70 - "VulnerabilityServiceServicer"
Cohesion: 0.50
...
### Community 75 - "number"
Cohesion: 0.50
Nodes (4): workloads, description, items, type
...
### Community 82 - "TestDatabaseConnection"
Cohesion: 0.67
Nodes (3): additionalProperties, type, databaseSchema

### Community 83 - "tables"
Cohesion: 0.67
Nodes (3): replicas, minimum, type

### Community 84 - ".UpdateEmail"
Cohesion: 0.20
Nodes (7): RemoveAvatarRequest, RemoveAvatarResponse, AuthService, Synchronous part of Refresh logic., AuthService handles user login, token refresh, and logout., Generates a JWT Access Token valid for 15 minutes., Helper to hash refresh tokens consistently.

### Community 87 - ".WatchCompanyUpdates"
Cohesion: 0.67
Nodes (3): tables, items, type

### Community 115 - ".WatchCompanyUpdates"
Cohesion: 0.18
Nodes (4): test_list_companies_owner_visibility(), test_list_users_owner_visibility(), WatchCompanyUpdatesRequest, WatchCompanyUpdatesResponse
...
- **Why does `User` connect `User` to `properties`, `agent_watcher.py`, `CompanyService`, `test_auth_service.py`, `🏗️ Core Models`, `.WatchCompanyUpdates`, `.UpdateEmail`, `properties`?**
  _High betweenness centrality (0.041) - this node is a cross-community bridge._
- **Why does `AuthService` connect `.UpdateEmail` to `properties`, `agent_watcher.py`, `User`, `.Logout`, `.Refresh`, `CompanyService`, `🏗️ Core Models`, `.SetupPassword`, `conftest.py`, `test_auth_service.py`, `.GetMe`, `.UpdatePassword`, `.UpdateProfile`?**
  _High betweenness centrality (0.039) - this node is a cross-community bridge._
- **Why does `$defs` connect `ping_pb2_grpc.py` to `properties`, `get_session_factory`, `🏗️ Modèles Principaux`, `properties`, `TestDatabaseConnection`, `properties`, `Changes Made`, `columns`, `AuthService`, `The Aegis AI Brain (Orchestrator)`, `Le Brain Aegis AI (Orchestrateur)`, `test_internal_auth.py`?**
